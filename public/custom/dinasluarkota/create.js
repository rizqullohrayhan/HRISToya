$(document).ready(function() {
    ...
    $(".btn-add-detail").on("click", function (event) {
        ...
        // Perbarui atribut for pada label sesuai dengan indeks baru
        rowBaru.find("label").each(function () {
            let forLabel = $(this).attr("for");

            if (forLabel) {
                let newId = forLabel.replace(/\d+/, rowIndex); // Ubah indeks di for
                $(this).attr("for", newId);
            }
        });
    ...
    $('.otorisasi-btn').on('click', function () {
        ...
        const otorisasiCell = $(`#otorisasi-table.${target} tbody tr td.${target}`);
