...
<div class="container">
    <div class="page-inner">
        <div class="page-header">
            <h4 class="page-title">Detail Surat Ijin</h4>
        </div>
        <div class="row">
            <div class="col-md-8">
                <div class="row">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-wrap gap-2 justify-content-start">
                            @if ($ruleUser && ($surat->diperiksa == null || auth()->user()->hasRole('ADM')))
                                <a href="{{ route('ijin.edit', $surat->id) }}" class="btn btn-warning">Edit</a>
                            @endif
                            ...
                            @if ($surat->disetujui && (auth()->user()->id == $surat->dibuat_id || (auth()->user()->hasRole('ADM'))))
                                <a href="{{ route('ijin.cetak', $surat->id) }}" target="_blank" class="btn btn-info">Cetak</a>
                            @endif

                            <a href="{{ route('ijin.index') }}" class="btn btn-danger">Kembali</a>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    window.Laravel = {
        otorisasiUrl: "{{ route('ijin.update.otoritas', $surat->id) }}",
        closeUrl: "{{ route('ijin.close', $surat->id) }}"
    };
    const url = "{{ route('ijin.show', $surat->id) }}";
